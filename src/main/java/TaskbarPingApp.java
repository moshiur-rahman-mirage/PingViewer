...
public class TaskbarPingApp {
    ...
    private static TaskbarPingApp instance;
...
//        System.out.println("Non-Heap: " + formatMemory(nonHeapUsage));
    ...
    private TaskbarPingApp(int g, int y,
        ...
        // Minimal frame to hold the icon in taskbar
        frame = new JFrame();
    ...
    public static void run(int g, int y,
        ...
        SwingUtilities.invokeLater(() -> instance = new TaskbarPingApp(
                g, y, gc, yc, rc, gtc, ytc, rtc, host));
    }
    ...
    private BufferedImage drawIcon(int latency) {
        ...
        try {
            ...
            int width = widthCache.computeIfAbsent(txt, k -> g2.getFontMetrics().stringWidth(k));
            g2.setColor(text);
            g2.drawString(txt, (ICON_SIZE - width) / 2, (ICON_SIZE + maxAscent) / 2 - 2);
            if (widthCache.size() > 100) widthCache.clear();
        } finally {
            g2.dispose();
        }
        return img;
